...
def getChineseCharacter(tag_code):
    return np.unicode(tag_code, 'gb2312')
...
def getDict(dir):
    ...
    pickle.dump(cdict, open('char_dict.txt', 'wb'))
    return cdict
...
def arg_parser():
    ...
    parser.add_argument("--gnt-dir", type=str, help="Absolute path of gnt dateset",
                        default=os.path.abspath("E:/Datasets/HWDB/sub/gnt"))
    parser.add_argument("--output-dir", type=str, help="Absolute path for storing pictures",
                        default=os.path.abspath("E:/Datasets/HWDB/sub"))
    parser.add_argument("--dict",type=bool,default=True)
...
def main(args):
    ...
    char_dict=getDict(trn_path)
    extractor(trn_path,trn_out_path,char_dict)
    extractor(tst_path,tst_out_path, char_dict)
